...
public class OrderManagerViewSwingImplTest extends AssertJSwingJUnitTestCase {
	...
	public void testCreateOrderButtonShouldDelegateToController() {
		...
		verify(orderController).createOrder(
				new Order("3", "Customer3", "Company3", "Product3", "Status3", "Description3", null, null));
	...
	public void testCreateOrderWithExistingIdShowsErrorMessage() {
		when(orderController.getOrderById("1")).thenReturn(new Order("1", "Existing Customer", "Existing Company",
				"Existing Product", "Existing Status", "Existing Description", null, null));

		window.button(JButtonMatcher.withText("Create Order")).click();

		window.dialog().textBox("idField").enterText("1");
		window.dialog().textBox("customerField").enterText("New Customer");
		window.dialog().textBox("companyField").enterText("New Company");
		window.dialog().textBox("productField").enterText("New Product");
		window.dialog().textBox("statusField").enterText("New Status");
		window.dialog().textBox("descriptionField").enterText("New Description");
	...
	public void testUpdateAndDeleteButtons() {
	    List<Order> orders = Arrays.asList(
	        new Order("1", "Customer1", "Company1", "Product1", "Status1", "Description1", "Test", "Test")
	    );
